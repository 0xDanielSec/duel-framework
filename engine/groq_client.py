...
def chat(
    ...
    Local Ollama calls always pass keep_alive=0 (force immediate unload after
    each call) instead of the server's ~5min default residency — Attacker and
    Defender models are often both several GB; observed OOM on a 16GB machine
    when qwen2.5:14b (9GB) tried to load while llama3.1:8b (7GB) was still
    resident from the previous call — see docs/scaling_v2_results.md. Reload
    cost per call (~10-30s) is noise next to multi-minute rounds. Groq calls
    have no local-memory concept and are unaffected.
    """
    ...
    if reasoning_effort is None:
        return client.chat(model=model, messages=messages, options=options, keep_alive=0)
    return client.chat(model=model, messages=messages, options=options, think=reasoning_effort, keep_alive=0)
